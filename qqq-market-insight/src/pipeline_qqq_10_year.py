...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    data["next_return"] = close.shift(-5) / close - 1
...
def run(download: bool = False, force: bool = False) -> dict[str, Any]:
    ...
    quality.update(data.attrs["quality"])
     ...
     # ---- เปลี่ยนการ Shift Label และการเว้นระยะ Train/Test ----
    label_dates = pd.Series(
        clean_ohlcv["Date"].shift(-5).to_numpy(), index=clean_ohlcv["Date"]
    ...
    save_eda(data.iloc[:split_index - 5])

    # 13.5. Chronological train/test split.
    train = data.iloc[:split_index - 5].copy() 
    ...
    if label_dates.iloc[split_index - 6] >= test["Date"].min(): # เช็คย้อนหลัง 6 วัน (5+1)
